...
namespace Graphs
{
    class Graph //the whole graph represented by list of neighbours
    {
        ...
        private Dictionary<Node, int> level;
        ...
        public int GetLevel(int start, int end)
        {
            level = new Dictionary<Node, int>();
            this.BFS(Nodes[start]);

            if (this.level.ContainsKey(Nodes[end]))
            {
                return this.level[Nodes[end]];
            } else
            {
                return -1;
            }
